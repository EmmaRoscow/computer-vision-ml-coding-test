...
class Dataset(object):
    ...
    def prepare_data(self):
        ...
        self.train_indices = np.flatnonzero(eval_data.loc[:,1] == 0).astype(int)
        self.val_indices = np.flatnonzero(eval_data.loc[:,1] == 1).astype(int)
    ...
    def get_image_minibatch(self, image_files, start_index, end_index):
    ...
    def get_batches(self, batch_size, validate=False):
        """
        Generate batches
        ...
        if validate:
            image_files = sorted(list(np.array(glob(
                os.path.join(self.dataset_path, self.img_dwnld_dir, '*.jpg')
                ))[self.val_indices]))
        ...
        self.shape = len(self.data_files), self.im_width, \
        self.im_height, self.im_channels

        current_index = 0
        while current_index + batch_size <= self.shape[0]:
            data_batch = self.get_image_minibatch(
                    self.data_files[current_index:current_index + batch_size], current_index, current_index + batch_size)

            current_index += batch_size

            yield data_batch / IMAGE_MAX_VALUE - 0.5

    def get_batch_labels(self, batch_size, validate=False):
        ...
        gender_labels = np.array(attr_data[{'Male'}].values)

        if validate:
            gender_labels = gender_labels[self.val_indices]

        else:
            gender_labels = gender_labels[self.train_indices]

        current_index = 0
        while current_index + batch_size <= self.shape[0]:
            data_batch = gender_labels[current_index:current_index + batch_size]
